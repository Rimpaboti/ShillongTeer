...
export default function AdminBetsLookupPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const q = query(
        collection(db, 'bets'),
        where('day', '==', new Date(date).toLocaleDateString()),
        where('gameSlot', '==', slot),
        where('range', '==', range),
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-2xl mb-8">
          ...
          <button
            ...
            className={`w-full py-3 rounded-md text-black font-semibold transition ${
              loading ? 'bg-yellow-100 cursor-not-allowed' : 'bg-yellow-400 hover:bg-yellow-500'
            }`}
          >
          ...
          <div className="bg-gray-800 p-6 rounded-lg shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {bets.map((bet) => (
                ...
                >
                  <p>
                    <span className="font-bold">User:</span> {bet.email}
                  </p>
                  <p>
