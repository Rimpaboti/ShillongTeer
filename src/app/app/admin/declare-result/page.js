...
export default function DeclareResultPage() {
  ...
  const handleDeclare = async () => {
    if (!day || !gameSlot || !range || winningNumber.trim() === '') {
    ...
    if (range === '0-9') {
      if (isNaN(winningNum) || winningNum < 1 || winningNum > 9) {
        alert('Winning number must be between 1 and 9.');
        return;
      }
    } else {
    ...
    try {
      ...
      // ✅ Step 3: Run transaction to update wallets + mark bets
      await runTransaction(db, async (transaction) => {
        for (const betDoc of betsSnap.docs) {
          const bet = betDoc.data();
          const isWinner = bet.number == winningNum;

          if (isWinner) {
            const multiplier = range === '0-9' ? 9 : 600;
            const winAmount = bet.amount * multiplier;

            const walletRef = doc(db, 'wallets', bet.uid);
            const walletSnap = await transaction.get(walletRef);

            let newBalance = winAmount;
